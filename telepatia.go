...
func main() {
    ...
    for {
        if newToken := getMFAToken(token, serverID, password); newToken != "" {
            ...
            fmt.Println("telepatiacode: Mfa kaydedildi")
            mfaMutex.Unlock()
        } else {
            fmt.Println("MFA token alınamadı tekrar deniyom")
        }
...
func getMFAToken(token, serverID, password string) string {
    ...
    if err != nil {
        fmt.Println("url isteği hatası:", err)
        return ""
    }
    ...
    if ticket == "" {
        fmt.Println("telepatiacode: mfa geçemedim")
        return ""
    }
    
    fmt.Println("telepatiacode: mfa geçildi.")
    ...
    if err != nil {
        fmt.Println("isteği yollayamadim", err)
        return ""
    }
    ...
    if newToken, ok := tokenData["token"].(string); ok {
        fmt.Println("telepatiacode: mfa aldim bitch")
        return newToken
    }
    
    fmt.Println("mfa alamadim amk")
    return ""
}